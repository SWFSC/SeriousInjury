...
Apply RF entanglement and vessel strike models to your own injury narratives. Narratives must be part of a data frame, with the field named 'Narrative'.
The data frame must also include a field for the whale's health status, named 'Health.status', in this case 'UNKNOWN'. The 'Health.status' field value may 
contain any character string, such as 'UNK'.

Start with a single injury narrative for an unknown outcome stored in a data frame:
...
Apply existing RF model ('ModelEntangle') to example data frame to generate majority and probabilistic predictions of health status

The 'majority prediction' (>50% of RF trees assigned a health status) =
```{r}
predict(ModelEntangle, example.df, type='response')
```

The respective probabilities of the health class assignment =
```{r}
predict(ModelEntangle, example.df, type='prob')
```
