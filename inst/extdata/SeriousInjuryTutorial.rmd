...
# View a portion of WhaleDataCovs
head(WhaleDataCovs[,c(3:6,28,31:49)])
```
...
```{r}
# Estimate injury classes for entanglements with unknown outcomes, based on data frame 'data.test.entangle'.
...
# Apply existing RF model ('ModelEntangle') to example.df to generate majority and probabilistic predictions of health status. The majority prediction ("DEAD.DECLINE" vs "RECOVERED") is determined by which class is predicted in >50% of RF trees. The number of trees (from a random forest of 1,000 trees) assigning the narrative to each respective health class is reflected in the "DEAD.DECLINE" and "RECOVERED" fractions returned.
...
example.df.with.predictions <- cbind.data.frame(majority.prediction, prob.prediction, example.df)
example.df.with.predictions
```
