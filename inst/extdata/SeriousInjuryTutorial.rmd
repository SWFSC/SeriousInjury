...
2. 'Health.status' includes one of three assessed conditions: a known death or health decline ('DEAD.DECLINE'), a whale that survives and recovers from its injuries >= 1 year post-interaction ('RECOVERED'), and data-poor cases for which the final health status is undetermined ('UNKNOWN').

3. 'CAUSE' attributes the main cause of injury to the following sources: entanglement = "EN", entrapment = "ET", and vessel strike = "VS"). Entanglements and entrapments (relatively rare) are both treated as forms of entanglements for purposes of injury modeling.
...
head(WhaleDataCovs[,c(1:3,6,31:49)])
```
...
Two models are used in the package SeriousInjury, an entanglement and a vessel strike model. Each is based on n = 1,000 RF classification trees. Model concepts are shown in Figures 1 and 2 below.
...
entangle.covariates = which(names(data.entangle)%in%c("anchored", "calf.juv", "constricting", "decline", 
...
Note the above Narrative involves an entanglement case. Yet, the data frame includes variables for vessel speed and size, 
which are not used in the RF entanglement model. Inclusion of a vessel size and speed in the appended data frame is due to
application of a text filter used to assign *all* covariates to *all records* initially, after which they are parsed out into
separate entanglement and vessel strike data frames. To be clear, the vessel covariate fields are ignored with application of
